...
bool set_timezone_utc(JsIsoDate* d)
{
  d->timezone_style = TimezoneStyle::ExplicitUtc;
  return true;
}

bool parse_timezone_plus(string_view& s, JsIsoDate* d)
{
  d->timezone_style = TimezoneStyle::ExplicitOffsetPlus;
  return parse_timezone_offset(s, d);
}

bool parse_timezone_minus(string_view& s, JsIsoDate* d)
{
...
bool maybe_parse_timezone(string_view& s, JsIsoDate* d) {
  assert(d->time_style != TimeStyle::None);
  if (consume(s, 'Z'))
    return set_timezone_utc(d);
  if (consume(s, '+'))
     return parse_timezone_plus(s, d);
  if (consume(s, '-'))
     return parse_timezone_minus(s, d);
  return true;
}
...
optional<JsIsoDate> parse_js_ios_date(string_view s) {
  ...
  if (!parse_date(s, &date) ||
      consume(s, 'T') && (!parse_time(s, &date) || !maybe_parse_timezone(s, &date)) ||
      !s.empty())
    return {};
