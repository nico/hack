...
int run(int64_t* progdat, const int N, int x, int y) {
  int64_t dat[N];
  memcpy(dat, progdat, N * sizeof(int64_t));
...
int main(void) {
  int64_t dat[4096];
  const int DATN = sizeof(dat) / sizeof(dat[0]);
  ...
  const int X = 0, Y = 0;
  #define CHECK(px, py) (run(dat, DATN, px, py) > 0)
