...
int bfs(uint8_t* grid, const int NX, const int NY, int dx, int dy) {
  int q[NX*NY][2];
...
move:
  ...
  if (i++ % 200000 == 0) {
    ...
    // bfs (dx, dy), abort if it hits a ? along the way.
    if (dx != -1) {
      int bfsr = bfs(&grid[0][0], NX, NY, dx, dy);
      if (bfsr != -1) {
        printf("%d\n", bfsr);
        exit(0);
      }
