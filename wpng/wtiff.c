...
void wtiff(int w, int h, const uint8_t* pix, FILE* f) {  // pix: rgba in memory
  ...
  // 0x116: rows per strip, 0x117: StripByteCounts
  ...
  tag = 0x102; type = 3; count = 4; val = 0x7a;  // bits per sample
  fwrite(&tag, 2, 1, f); fwrite(&type, 2, 1, f);
  fwrite(&count, 4, 1, f); fwrite(&val, 4, 1, f);
  ...
  tag = 0x115; type = 3; count = 1; sval = 4;  // samples per pixel
  fwrite(&tag, 2, 1, f); fwrite(&type, 2, 1, f);
  fwrite(&count, 4, 1, f); fwrite(&sval, 2, 1, f); fwrite("\0\0", 1, 2, f);
  ...
  // strip offsets
  for (int y = 0; y < h; ++y) {
    val = 0x7a + 8 + h*6 + y * w * 4; fwrite(&val, 4, 1, f);
  }
  // strip sizes
  for (int y = 0; y < h; ++y) {
